...
void PlayerbotHolder::HandlePlayerBotLoginCallback(QueryResult *dummy, SqlQueryHolder * holder)
{
    ...
    WorldSession *botSession = new WorldSession(botAccountId, nullptr, SEC_PLAYER, 2, 0, LOCALE_enUS);
    ...
    if (masterSession)
    {
        ...
        ch.PSendSysMessage("Your are not allowed to control bot %s", bot->GetName());
    }
